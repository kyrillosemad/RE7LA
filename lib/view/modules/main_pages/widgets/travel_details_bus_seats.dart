  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemCount: widget
            .travelModel.coaches![widget.controller.coachNumber].seats!.length,
        itemBuilder: (context, index) {
          Services services = Get.find<Services>();
          var seats = widget.controller.seats[index];
          return GestureDetector(
            ...
            child: Container(
              decoration: BoxDecoration(
                color:
                    const Color.fromARGB(255, 224, 224, 224).withOpacity(0.8),
                ...
                children: [
                  Center(
                    child: Text(
                      "Seat ${index + 1}",
                      style: TextStyle(
                        color: widget.controller.selectedSeat.any((seat) =>
                                seat['seatId'] == seats.seatId.toString())
                            ? AppColor.primaryColor
                            : seats.seatStatus == 0
                                ? AppColor.grey
                                : seats.ownerId.toString() ==
                                        services.sharedPref!.getString("userId")
                                    ? Colors.green
                                    : Colors.red,
                        fontSize: 10.sp,
                      ),
                  ...
                  Icon(
                    Icons.event_seat,
                    size: 40,
                    color: widget.controller.selectedSeat.any(
                            (seat) => seat['seatId'] == seats.seatId.toString())
                        ? AppColor.primaryColor
                        : seats.seatStatus == 0
                            ? AppColor.grey
                            : seats.ownerId.toString() ==
